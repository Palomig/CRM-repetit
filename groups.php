...
<div x-data="groupsApp()">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 space-y-4 sm:space-y-0">
        <div>
            <h1 class="text-3xl font-bold text-white">Группы</h1>
            <p class="text-gray-400 mt-1">Управление учебными группами</p>
        </div>
            ...
            <div class="bg-gray-800 border border-gray-700 rounded-lg shadow hover:shadow-lg transition-shadow">
                <div class="p-6">
                    <div class="flex items-start justify-between mb-4">
                        <div class="flex-1">
                            <h3 class="text-lg font-bold text-white" x-text="group.name"></h3>
                            <p class="text-sm text-gray-400 mt-1" x-text="group.subject"></p>
                        </div>
                        <div class="flex space-x-2">
                            <button @click="editGroup(group)" class="text-indigo-400 hover:text-indigo-300">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button @click="deleteGroupConfirm(group.id)" class="text-red-400 hover:text-red-300">
                    ...
                    <div class="mb-4">
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-sm text-gray-400">Заполненность:</span>
                            <span class="text-sm font-bold"
                                  :class="group.current_students >= group.max_students ? 'text-red-400' : 'text-green-400'"
                                  x-text="group.current_students + '/' + group.max_students">
                            </span>
                        </div>
                        <div class="w-full bg-gray-700 rounded-full h-2">
                            <div class="h-2 rounded-full transition-all"
                                 :class="group.current_students >= group.max_students ? 'bg-red-500' : 'bg-green-500'"
                    ...
                    <div class="space-y-2 mb-4">
                        <div class="text-sm text-gray-400">
                            ...
                            <span class="text-gray-200" x-text="group.teacher_name"></span>
                        </div>
                        <div class="text-sm text-gray-400">
                            <i class="fas fa-door-open mr-2"></i>
                            <span class="text-gray-200" x-text="group.room_name"></span>
                        </div>
                        <div class="text-sm text-gray-400">
                            <i class="fas fa-ruble-sign mr-2"></i>
                            <span class="text-gray-200" x-text="formatMoney(group.price) + ' / урок'"></span>
                        </div>
                    </div>

                    <!-- Список учеников -->
                    <div x-show="group.students && group.students.length > 0" class="pt-4 border-t border-gray-700">
                        <p class="text-xs font-medium text-gray-400 mb-2">Ученики:</p>
                        <div class="space-y-1">
                            <template x-for="student in group.students" :key="student.id">
                                <div class="text-xs text-gray-200 flex items-center">
                    ...
                    <div class="mt-4 pt-4 border-t border-gray-700">
                        <span class="inline-block px-2 py-1 text-xs font-medium rounded-full"
                              :class="group.status === 'active' ? 'bg-green-500/20 text-green-400' : 'bg-gray-500/20 text-gray-400'"
        ...
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-gray-800 border border-gray-700 rounded-lg shadow-xl max-w-lg w-full p-6">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-xl font-bold text-white" x-text="modalMode === 'create' ? 'Создать группу' : 'Редактировать группу'"></h3>
                ...
                <form @submit.prevent="saveGroup">
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-200 mb-2">Название группы *</label>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-200 mb-2">Максимум учеников</label>
                                <input type="number" x-model="form.max_students" min="1" max="12" class="w-full px-3 py-2 bg-gray-700 border border-gray-600 text-white placeholder-gray-400 rounded-lg focus:ring-2 focus:ring-blue-500">
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-200 mb-2">Стоимость (₽)</label>
